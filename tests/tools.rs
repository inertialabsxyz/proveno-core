...
fn run_with_host(src: &str, host: MockHost, config: VmConfig) -> Result<proveno::VmOutput, VmError> {
...
fn run_with_tape(src: &str, tape: OracleTape, config: VmConfig) -> Result<proveno::VmOutput, VmError> {
    let block = parse(src).expect("parse failed");
    let program = compile(&block).expect("compile failed");
    verify(&program).expect("verify failed");
